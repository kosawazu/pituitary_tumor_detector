    ...
    while True:
        ...
        while cap.isOpened():
            if not is_paused:
                ...
                # アスペクト比を維持してウィンドウサイズに合わせてリサイズ
                try:
                    win_x, win_y, win_w, win_h = cv2.getWindowImageRect('Segmentation Result')
                    h, w = segmentation_frame.shape[:2]
                    aspect_ratio = w / h

                    # ウィンドウの中で表示できる最大サイズをアスペクト比を保って計算
                    ...
                    resized_frame = cv2.resize(segmentation_frame, (new_w, new_h), interpolation=cv2.INTER_LINEAR)
                    # 空白部分を黒で埋めたウィンドウフレーム作成（中央配置）
                    ...
                    cv2.imshow('Segmentation Result', display_frame)
                except:
                    # エラーが起きたら通常表示（初回など）
...
def process_frame(frame, model, device, transform):
    ...
    original_image = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))  # OpenCVからRGB変換
